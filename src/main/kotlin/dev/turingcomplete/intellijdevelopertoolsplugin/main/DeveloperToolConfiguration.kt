package dev.turingcomplete.intellijdevelopertoolsplugin.main
...
class DeveloperToolConfiguration(
  ...
  private fun <T : Any> reuseExistingProperty(property: PropertyContainer): ValueProperty<T> {
    if ((property.type == PropertyType.INPUT && !DeveloperToolsApplicationSettings.instance.saveInputs)
      || (property.type == PropertyType.CONFIGURATION && !DeveloperToolsApplicationSettings.instance.saveConfigurations)
      || (property.type == PropertyType.SENSITIVE && !DeveloperToolsApplicationSettings.instance.saveSensitiveInputs)
    ) {
