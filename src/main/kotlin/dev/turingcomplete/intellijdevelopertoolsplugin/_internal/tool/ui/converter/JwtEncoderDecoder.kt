...
import dev.turingcomplete.intellijdevelopertoolsplugin._internal.tool.ui.converter.JwtEncoderDecoder.SignatureAlgorithm.entries
...
import dev.turingcomplete.intellijdevelopertoolsplugin._internal.tool.ui.converter.JwtEncoderDecoder.StandardClaim.entries
...
internal class JwtEncoderDecoder(
  ...
  private class ClaimFeatureUnixTimestampGutterIconRenderer(
    ...
    override fun getTooltipText(): String {
      ...
      val diff = DateFormatUtil.formatBetweenDates(unixTimestampSeconds.times(1000), System.currentTimeMillis())
  ...
  private class Jwt(
    ...
    fun encodeJwt() {
      ...
      val headerJson = try {
        val headerJson = objectMapper.readTree(header.get())
      ...
      val payloadJson = try {
        objectMapper.readTree(payload.get())
      } catch (e: Exception) {
      ...
      if (headerErrorHolder.isSet() || payloadErrorHolder.isSet()) {
        ...
        val encodedPayload = urlEncoder.encode(objectMapper.writeValueAsString(payloadJson!!).encodeToByteArray()).decodeToString()
  ...
  private class Signature(
    ...
    fun compute(encodedHeader: String, encodedPayload: String): String? {
      ...
      return try {
        ...
        ExtendedJsonWebSignature().apply {
