...
internal class DeveloperToolEditor(
  ...
  private var softWraps = configuration.register("${context.id}-${id}-softWraps", true, CONFIGURATION)
  private var showSpecialCharacters = configuration.register("${context.id}-${id}-showSpecialCharacters", false, CONFIGURATION)
  private var showWhitespaces = configuration.register("${context.id}-${id}-showWhitespaces", false, CONFIGURATION)
