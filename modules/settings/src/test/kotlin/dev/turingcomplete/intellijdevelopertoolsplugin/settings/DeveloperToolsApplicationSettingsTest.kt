...
class DeveloperToolsApplicationSettingsTest {
  ...
  fun `test that the settings names have not been changed`(
    ...
    assertThat(
        getSettings(developerToolsApplicationSettings).settingsContainer().settingProperties.keys
      )
  ...
  fun `test restoring from XML`(
    ...
    val modifiedAttributes =
      settingsContainer.settingProperties
        .map { (settingName, property) -> settingName to property.get().modifyValue() }
        .toMap()

    val xmlAttributes =
      modifiedAttributes
        .map { (settingName, modifiedValue) ->
          val persistentValue =
            when (modifiedValue) {
    ...
    settingsContainer.settingProperties.forEach { (settingName, property) ->
      assertThat(property.isModified()).describedAs("$xmlElementName - $settingName").isTrue
  ...
  fun `test persisting to XML`(
    ...
    val settingsElement: Element =
      actualState.content.find { (it as Element).name == xmlElementName } as Element
  ...
  private fun Any.modifyValue(): Any =
    when (this) {
      is Boolean -> !this
      is Int -> this + 1
      is Enum<*> -> {
