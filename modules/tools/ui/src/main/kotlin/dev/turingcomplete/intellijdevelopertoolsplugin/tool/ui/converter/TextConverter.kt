...
abstract class TextConverter(
  ...
  private var lastActiveInput = AtomicProperty(ActiveInput.SOURCE)
  private val toSourceActive = PropertyComponentPredicate(lastActiveInput, ActiveInput.TARGET)
  ...
  private fun liveTransformToLastActiveInput() {
    if (liveConversion.get()) {
      ...
      when (lastActiveInput.get()) {
        ActiveInput.SOURCE -> transformToTarget()
        ActiveInput.TARGET -> transformToSource()
      }
    }
  }
