...
import dev.turingcomplete.intellijdevelopertoolsplugin.common.ScrollPaneBuilder
...
class UnitsConverter(
  ...
  private var lastSelectedUnitConverterIndex by
    configuration.register(
      "lastSelectedUnitConverterIndex",
      DEFAULT_LAST_SELECTED_UNIT_CONVERTER_INDEX,
    )

  private val unitConverters: List<UnitConverter> =
    listOf(
  ...
  override fun Panel.buildUi() {
    unitConvertersTabbedPanel =
      TabbedPaneWrapper(parentDisposable).apply {
        val tabBorder = Borders.emptyTop(12)
        unitConverters.forEach { converter ->
          val component = converter.createComponent().apply { border = tabBorder }
          addTab(
            converter.title,
        ...
        val initialSelectedUnitConverterIndex =
          if (lastSelectedUnitConverterIndex < this.tabCount) {
            lastSelectedUnitConverterIndex
          } else {
            DEFAULT_LAST_SELECTED_UNIT_CONVERTER_INDEX
          }
  ...
  class Factory : DeveloperUiToolFactory<UnitsConverter> {
    ...
    ): ((DeveloperToolConfiguration) -> UnitsConverter) = { configuration ->
      UnitsConverter(configuration, parentDisposable)
    }
  }
