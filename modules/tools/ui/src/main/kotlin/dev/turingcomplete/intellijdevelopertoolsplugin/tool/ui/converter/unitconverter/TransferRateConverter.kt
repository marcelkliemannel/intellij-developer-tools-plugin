...
class TransferRateConverter(
  ...
  private val timeDimension =
    configuration.register("${CONFIGURATION_KEY_PREFIX}timeDimension", DEFAULT_TIME_DIMENSION)
  ...
  private val useCombinedAbbreviationNotation =
    configuration.register(
      "${CONFIGURATION_KEY_PREFIX}useCombinedAbbreviationNotation",
      DEFAULT_USE_COMBINED_ABBREVIATION_NOTATION,
    )
  private val bitTransferRateValue =
    configuration.register(
      "${CONFIGURATION_KEY_PREFIX}bitTransferRateValue",
      ZERO,
      INPUT,
      DEFAULT_BIT_TRANSFER_RATE_VALUE,
    )
      ...
      group(numberSystem.title, true) {
          transferRateProperties
            ...
            .forEach { transferRateDataUnitProperty ->
              row {
                  val formattedLabel =
                    label("").bindText(transferRateDataUnitProperty.inputTitle).gap(RightGap.SMALL)
  ...
  override fun doSync() {
    ...
    transferRateProperties.forEach { it.sync() }
  }
  ...
  private fun convertByInputFieldChange(
    ...
    if (inputFieldTransferRateProperty != bitTransferRateProperty) {
      ...
      bitTransferRateValue.set(
        inputFieldTransferRateProperty.dataUnit.toBits(inputValue, mathContext)
      )
    } else {
  ...
  private fun createTransferRateProperties() =
    dataUnits.map {
      if (it == bitDataUnit) {
        TransferRateProperty(it, bitTransferRateValue, bitTransferRateValue.get().toFormatted()) {
            createTitle(it, timeDimension)
          }
