...
class AddOpenMainDialogActionToMainToolbarTask(
  ...
  override fun run() {
    if (
      mainToolbarRightActionGroup != null &&
        !mainToolbarRightActionGroup.containsAction(openMainDialogAction)
    ) {
      mainToolbarRightActionGroup.add(
        openMainDialogAction,
        Constraints(Anchor.BEFORE, "SearchEverywhere"),
      )
    }

    if (
      mainToolbarActionGroup != null && !mainToolbarActionGroup.containsAction(openMainDialogAction)
    ) {
  ...
  companion object {
     ...
     * - The [DefaultActionGroup.add] in [AddOpenMainDialogActionToMainToolbarTask] will not add the
     *   action if the toolbar does not exist. Each toolbar exists only either in the new or old UI.
     *   So, the action will only be added to the toolbar of the currently active UI.
     * - If the user activity removed the action from the toolbar, the
     *   [AddOpenMainDialogActionToMainToolbarTask] will still add the action, but it will be hidden
     *   (this due to the working toolbar action mechanism). Therefore, the
     *   [DefaultActionGroup.containsAction] returns true even though the action is not visible.
     */
    fun createIfAvailable(): AddOpenMainDialogActionToMainToolbarTask? {
      ...
      val mainToolbarRightActionGroup =
        ActionManager.getInstance()
          .getAction(IdeActions.GROUP_MAIN_TOOLBAR_RIGHT)
          ?.safeCastTo<DefaultActionGroup>() // New UI
      val mainToolbarActionGroup =
        ActionManager.getInstance()
          .getAction(IdeActions.GROUP_MAIN_TOOLBAR)
          ?.safeCastTo<DefaultActionGroup>() // Old UI
